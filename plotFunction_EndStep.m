...
if M.plot_pars.plot_location
    ...
    if M.plot_pars.use_carveout
        th = (M.t*0.0001388888889 - 0.05)*pi; % shift the carve out so it is not head on (one full revolution every 10 days)
        for axi = 1:8
    ...
    view(M.fig.ax(M.fig.scatter_ind), M.plot_pars.m*[cos(M.t*0.0001388888889*pi)*0.8660,sin(M.t*0.0001388888889*pi)*0.8660,0.5]) % full rotation every 10 days
end
...
%% tracked quantities
%% population plot
for i = 1:length(M.fig.population_plots)
    M.fig.population_plots(i).XData(end+1) = M.t / 1440;
end
M.fig.population_plots(1).YData(end+1) = M.NT;
M.fig.population_plots(2).YData(end+1) = M.NI;

%% subpopulations proportion plot
for i = 1:length(M.fig.subpop_plots)
    M.fig.subpop_plots(i).XData(end+1) = M.t / 1440;
end
...
%% receptor plot
M.fig.receptor_plots(1).XData(end+1) = M.t / 1440;
M.fig.receptor_plots(1).YData(end+1) = M.tracked.phiD_mean(M.i) / M.fgfr3.gammaT;

%% drug plot
for i = 1:length(M.fig.afgfr3_plots)
    M.fig.afgfr3_plots(i).XData(end+1) = M.t / 1440;
end
...
for i = 1:length(M.fig.apd1_plots)
    M.fig.apd1_plots(i).XData(end+1) = M.t / 1440;
end
...
%% event plots
for i = 1:length(M.fig.events_plots)
    M.fig.events_plots(i).XData(end+1) = M.t / 1440;
end
M.fig.events_plots(1).YData(end+1) = 1440 * sum(M.tracked.tum_apop(M.i,:,:),2:3) / (M.NT*M.dt);
M.fig.events_plots(2).YData(end+1) = 1440 * M.tracked.tum_contact_inhibition(M.i) / (M.NT*M.dt);
M.fig.events_plots(3).YData(end+1) = 1440 * M.tracked.imm_recruit(M.i) / M.dt;
M.fig.events_plots(4).YData(end+1) = 1440 * M.tracked.imm_prolif(M.i) / (M.NI*M.dt);
M.fig.events_plots(5).YData(end+1) = 1440 * M.tracked.imm_contact_inhibition(M.i) / (M.NI*M.dt);
M.fig.events_plots(6).YData(end+1) = 1440 * M.tracked.imm_aicd(M.i) / (M.NI*M.dt);

%% interaction plots
for ii = 1:length(M.fig.interaction_plots)
    M.fig.interaction_plots(ii).XData(end+1) = M.t / 1440;
end
M.fig.interaction_plots(1).YData(end+1) = 1440 * M.tracked.imm_cleared(M.i,1,1) / (sum(~M.ha_log & ~M.mut_log)*M.dt);
M.fig.interaction_plots(2).YData(end+1) = 1440 * M.tracked.imm_cleared(M.i,2,1) / (sum(~M.ha_log & M.mut_log)*M.dt);
M.fig.interaction_plots(3).YData(end+1) = 1440 * M.tracked.imm_cleared(M.i,1,2) / (sum(M.ha_log & ~M.mut_log)*M.dt);
M.fig.interaction_plots(4).YData(end+1) = 1440 * M.tracked.imm_cleared(M.i,2,2) / (sum(M.ha_log & M.mut_log)*M.dt);
M.fig.interaction_plots(5).YData(end+1) = 1440 * M.tracked.deactivations(M.i,1) / (sum(unengaged_log)*M.dt);
M.fig.interaction_plots(6).YData(end+1) = 1440 * M.tracked.deactivations(M.i,2) / (sum(slow_log)*M.dt);
M.fig.interaction_plots(7).YData(end+1) = 1440 * M.tracked.deactivations(M.i,3) / (sum(fast_log)*M.dt);
