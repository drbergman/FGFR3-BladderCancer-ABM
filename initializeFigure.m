...
%% scatter plot
tumor_colors = M.plot_pars.tumor_colors;
tumor_colormap = flipud(winter);
immune_colors = M.plot_pars.immune_colors;
unengaged_color = M.plot_pars.unengaged_color;
deactivated_color = M.plot_pars.deactivated_color;
...
xtick_vals = xtick_vals / 1440; % plot in days
symbols = {'I','^','.','C'};
vals = [0,1,2,Inf];
...
M.fig.population_plots(1) = plot(M.t / 1440,M.setup.N0,'w-','LineWidth',2,'DisplayName','N_T');
M.fig.population_plots(2) = plot(M.t / 1440,M.setup.NI0,'b-','LineWidth',2,'DisplayName','N_I');
...
M.fig.subpop_plots(1) = plot(M.t / 1440,M.setup.prop_ha0,'w','LineWidth',2,'DisplayName','HA of Tumor');
M.fig.subpop_plots(2) = plot(M.t / 1440,M.setup.prop_mut0,'r','LineWidth',2,'DisplayName','FGFR3 Muts of Tumor');
M.fig.subpop_plots(3) = plot(M.t / 1440,M.setup.NI0/(M.setup.NI0+M.setup.N0),'b','LineWidth',2,'DisplayName','CTLs of All Cells');
...
M.fig.events_plots(1) = plot(M.t / 1440,0,'w','LineWidth',2,'DisplayName','Tumor Apoptosis');
M.fig.events_plots(2) = plot(M.t / 1440,0,'w:','LineWidth',2,'DisplayName','Tumor Contact Inhibitions');
M.fig.events_plots(3) = plot(M.t / 1440,0,'b','LineWidth',2,'DisplayName','Immune Recruit');
M.fig.events_plots(4) = plot(M.t / 1440,0,'b--','LineWidth',2,'DisplayName','Immune Prolif');
M.fig.events_plots(5) = plot(M.t / 1440,0,'b:','LineWidth',2,'DisplayName','Immune Contact Inhibitions');
M.fig.events_plots(6) = plot(M.t / 1440,0,'Color',[0.3,0.3,0.9],'LineStyle','-','LineWidth',2,'DisplayName','Immune AICD');
...
M.fig.interaction_plots(1) = plot(M.t / 1440,0,'Color',tumor_colors(1,:),'LineStyle',':','LineWidth',2,'DisplayName','LA Non-Mut IC');
M.fig.interaction_plots(2) = plot(M.t / 1440,0,'Color',tumor_colors(2,:),'LineStyle','-','LineWidth',2,'DisplayName','LA Mut IC');
M.fig.interaction_plots(3) = plot(M.t / 1440,0,'Color',tumor_colors(3,:),'LineStyle',':','LineWidth',2,'DisplayName','HA Non-Mut IC');
M.fig.interaction_plots(4) = plot(M.t / 1440,0,'Color',tumor_colors(4,:),'LineStyle','-','LineWidth',2,'DisplayName','HA Mut IC');
M.fig.interaction_plots(5) = plot(M.t / 1440,0,'Color',unengaged_color,'LineWidth',2,'DisplayName','Unengaged Deactivations');
M.fig.interaction_plots(6) = plot(M.t / 1440,0,'Color',immune_colors(2,:),'LineWidth',2,'DisplayName','Slow-killing Deactivations');
M.fig.interaction_plots(7) = plot(M.t / 1440,0,'Color',immune_colors(1,:),'LineWidth',2,'DisplayName','Fast-killing Deactivations');
...
M.fig.receptor_plots(1) = plot(M.t / 1440,0,'w','LineWidth',2,'DisplayName','\phi_D');
M.fig.receptor_plots(2) = plot(M.t / 1440,0,'g','LineWidth',2,'DisplayName','Checkpoint');
...
if M.fgfr3.n_regions==1
    c = spring(2);
    M.fig.afgfr3_plots(1) = plot(M.t / 1440,0,'Color',c(2,:),'LineStyle','-','LineWidth',2,'DisplayName','Free aFGFR3 (R1)');
else
    c = spring(3);
    M.fig.afgfr3_plots(1) = plot(M.t / 1440,0,'Color',c(2,:),'LineStyle','-','LineWidth',2,'DisplayName','Free aFGFR3 (R1)');
    M.fig.afgfr3_plots(2) = plot(M.t / 1440,0,'Color',c(3,:),'LineStyle','-','LineWidth',2,'DisplayName',sprintf('Free aFGFR3 (R%d)',M.fgfr3.n_regions));
end

M.fig.afgfr3_plots(end+1) = plot(M.t / 1440,0,'Color',c(1,:),'LineStyle','-','LineWidth',2,'DisplayName','aFGFR3 in Circ');

if M.checkpoint.n_regions==1
    c = winter(2);
    M.fig.apd1_plots(1) = plot(M.t / 1440,0,'Color',c(2,:),'LineStyle','-.','LineWidth',2,'DisplayName','Free aPD1 (R1)');
else
    c = winter(3);
    M.fig.apd1_plots(1) = plot(M.t / 1440,0,'Color',c(2,:),'LineStyle','-.','LineWidth',2,'DisplayName','Free aPD1 (R1)');
    M.fig.apd1_plots(2) = plot(M.t / 1440,0,'Color',c(3,:),'LineStyle','-.','LineWidth',2,'DisplayName',sprintf('Free aPD1 (R%d)',M.checkpoint.n_regions));
end

M.fig.apd1_plots(end+1) = plot(M.t / 1440,0,'Color',c(1,:),'LineStyle','-.','LineWidth',2,'DisplayName','aPD1 in Circ');
