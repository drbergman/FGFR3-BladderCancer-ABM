...
R = reshape(sol.y(:,end),szR); % store the end value to output and reshae
...
dimerization = [0,1,0].*(-M.fgfr3.kf*x(1,:,:).^2 + M.fgfr3.kr*x(2,:,:)); % dimerization and backwards reaction
dimer_recycling = M.fgfr3.kp*x(2,:,:); % recycling of dimers
monomer_inhibitor = -M.fgfr3.k_on_R*x(1,:,:).*x(3,:,:) + M.fgfr3.k_off_R*x(4,:,:); % FGFR3 reacting with aFGFR3
complex_dimerization = [0,1,0].*(-M.fgfr3.kf*x(4,:,:).^2 + M.fgfr3.kr*x(5,:,:)); % dimerization of these complexes (and the backwards reaction)
complex_recycling = M.fgfr3.kp*x(5,:,:); % recycling of monomer-inhibitor dimers
...
switch M.fgfr3.model_version
    case "GlobalVLocal"
        % free FGFR3
        dx(1,:,:) = 2*(dimerization+dimer_recycling) + monomer_inhibitor; %
        % dx(1,:,:) = 2*(dimerization+dimer_recycling+complex_recycling+dimer_inhibitor_recycling) + monomer_inhibitor; % if we will follow Kamaldeen's model exactly

        % dimerized FGFR3
        dx(2,:,:) = -(dimerization+dimer_recycling) + dimer_inhibitor;
        ...
        dx(4,:,:) = -monomer_inhibitor + 2*(complex_dimerization+complex_recycling);

        % aFGFR3-FGFR3 dimer
        dx(5,:,:) = -(complex_dimerization+complex_recycling); % dimerization of aFGFR3-FGFR3 complexes (and the backwards reaction)

        % aFGFR3-dimer complex
        dx(6,:,:) = -dimer_inhibitor;

    case "Kamaldeen"
        dimer_inhibitor_recycling = M.fgfr3.kp*x(6,:,:); % dimer-inhibitor complex recycling

        % free FGFR3
        dx(1,:,:) = 2*(dimerization+dimer_recycling+complex_recycling+dimer_inhibitor_recycling) + monomer_inhibitor;
        ...
        % aFGFR3-FGFR3 complex
        dx(4,:,:) = 2*complex_dimerization - monomer_inhibitor;

        % aFGFR3-FGFR3 dimer
        dx(5,:,:) = -(complex_dimerization+complex_recycling); % dimerization of aFGFR3-FGFR3 complexes (and the backwards reaction)

        % aFGFR3-dimer complex
        dx(6,:,:) = -(dimer_inhibitor+dimer_inhibitor_recycling);

end

assert(all(abs(sum(dx.*[1;2;0;1;2;2]))<1e-10,"all"))

dx = dx(:);

%% check conservation of total receptors
% 