...
for ImIter=1:M.Nsteps_Imm
    ...
    temp_isf = M.immune_stimulatory_factor.concentration(M.immunes(alive_ind(unengaged_and_activated_subalive_log),M.I.ind));
    temp_isf = (M.immune_pars.isf_prolif_ec50 ./ temp_isf).^M.immune_pars.isf_prolif_hill_coefficient;

    rate_matrix_imm = zeros(n_alive,6);
    % 1: proliferation
    rate_matrix_imm(unengaged_and_activated_subalive_log,1) = M.immune_pars.prolif_rate .* ( 1 + (M.immune_pars.isf_prolif_saturation_factor-1)./(1+temp_isf) ); % only proliferate if no timer within [0,M.dt_Imm)
    % 2: apoptosis
    rate_matrix_imm(:,2) = M.immune_pars.apop_rate; % immune apoptosis
    if M.immune_pars.is_exhausted_apoptosis_rate_different
        rate_matrix_imm(~activated_subalive_log,2) = M.immune_pars.apop_rate_exhausted;
    end
    % 3: movement
    rate_matrix_imm(unengaged_and_activated_subalive_log,3) = M.immune_pars.move_rate/M.immune_pars.steps_per_move; % movement of any kind
    % 4: conjugation
    rate_matrix_imm(unengaged_and_activated_subalive_log,4) = M.immune_pars.conjugation_rate; % conjugation attempt probability
    % 5: exhaustion
    rate_matrix_imm(:,5) = temp_deactivation_rate; % deactivation of immune cells
    % 6: AICD
    rate_matrix_imm(unengaged_and_activated_subalive_log & M.immunes(alive_ind,M.I.immune_seek_time) > M.immune_pars.time_to_seek,6) = M.immune_pars.aicd_rate; % rate of AICD due to not finding tumor cells
